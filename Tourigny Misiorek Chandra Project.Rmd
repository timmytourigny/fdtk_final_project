...
# Plot and fit a linear model for Total Deaths
model1 <- lm(aggregated_data$Total_Deaths ~ aggregated_data$Year)
plot(aggregated_data$Year, aggregated_data$Total_Deaths,
     xlab = "Year",
     ylab = "Total Deaths (per 100,000 people)",
...
# Fit linear models for each pollution type
...
# Plot Indoor, PM, and Ozone pollution deaths with regression lines
plot(aggregated_data$Year, aggregated_data$Indoor_Deaths,
     lwd = 2,
     xlab = "Year",
     ylab = "Deaths (per 100,000 people)",
...
legend("topright",
       ...
       col = c("red", "green", "blue"),
       lwd = 2,
       lty = c(1, 1, 1), # Line type for solid pollution lines
       bty = "n")
...
t_test_result <- t.test(
  Total_Deaths ~ Status, 
  data = data, 
  var.equal = FALSE, # Use Welch's t-test assuming unequal variances
  na.action = na.omit
)
print(t_test_result)
